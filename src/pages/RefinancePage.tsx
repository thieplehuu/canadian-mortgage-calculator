...
export default function RefinancePage() {
  ...
  const onChangeAmortization = (item: any) => {
    setAmotization(item);
    setResult(
      calculateMortgage(maxAmount, rate, item.value, paymentPeriod.value),
    );
  };
  ...
  const onChangePaymentPeriod = (item: any) => {
    ...
    setResult(
      calculateMortgage(maxAmount, rate, amortization.value, item.value),
    );
  };
      ...
      <KeyboardAwareScrollView
        ...
        <View style={AppStyle.StyleMain.container}>
          ...
          <View
            ...
            <View style={{alignContent: 'flex-start'}}>
              <Text>{moneyRound(minAmount, true, true)}</Text>
            </View>
            <View style={{alignSelf: 'stretch'}}></View>
            <View style={{alignContent: 'flex-end'}}>
              <Text>{moneyRound(homeValue * 0.8, true, true)}</Text>
            </View>
          </View>
          <PercentTextInput
            label="Rates"
